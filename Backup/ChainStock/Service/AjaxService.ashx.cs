	...
	public class AjaxService : IHttpHandler, IRequiresSessionState
	{
		...
		public void ChangeCard()
		{
			...
			try
			{
				...
				else if (bllMem.Exists(modelMem.MemID, newCard, "", "") != 1)
				{
					flag = -4;
				}
		...
		public void AddSubMem()
		{
			...
			try
			{
				...
				Chain.BLL.SubMem bllSubMem = new Chain.BLL.SubMem();
				if (bllSubMem.IsHasMemCard(subCardNumber))
				{
					flag = -2;
					this.Context.Response.Write(flag);
					return;
				}
				if (bllSubMem.IsHasMemCard(subMemMobile))
				{
					flag = -3;
					this.Context.Response.Write(flag);
					return;
		...
		public void Wx_MemRegister()
		{
			...
			try
			{
				...
				int shopid = bllShop.GetShopIDByWhere(" IsMain=1");

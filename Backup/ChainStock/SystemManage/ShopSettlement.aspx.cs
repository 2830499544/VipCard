...
{
	public class ShopSettlement : PageBase
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			if (!base.IsPostBack)
			{
				...
				if (base.Request.QueryString["PID"] != null)
				{
					...
					if (pid == 167)
					{
						this.ltlTitle.Text = "主页  >  联盟商管理  >  联盟商金额结算";
					}
					else if (pid == 165)
					{
						this.ltlTitle.Text = "主页  >  商家管理  >  商家金额结算";
					}
				}
			}
		}
		...
		private void Get_ParameterList(string strSql)
		{
			...
			bllSysShopSettlement.UpDateSettlement();
			int Counts = this.NetPagerParameter.RecordCount;
			string join = " on SysShopSettlement.OutShopID = SysShop.ShopID ";
			DataTable db = bllSysShopSettlement.GetListSP(this.NetPagerParameter.PageSize, this.NetPagerParameter.CurrentPageIndex, join, out Counts, new string[]
			{
				strSql
			}).Tables[0];

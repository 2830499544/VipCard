...
{
	public class ShopGroupList : PageBase
	{
		...
		private void GetGroupList()
		{
			...
			string strSql = " 1=1";
			strSql += string.Format("  and GroupType=3    and (IsPublic = '1'  or CreateUserID in (select UserID from sysUser where UserShopID={0}))", this._UserShopID);
		...
		protected void gvGroupList_ItemDataBound(object sender, RepeaterItemEventArgs e)
		{
			if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
				...
				if (dr["ParentIDStr"].ToString().IndexOf("/" + 3 + "/") < 0)
				{

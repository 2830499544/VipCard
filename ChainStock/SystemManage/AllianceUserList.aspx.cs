...
{
	public class AllianceUserList : PageBase
	{
		...
		protected AllianceQuickSearch QuickSearch1;

		protected void Page_Load(object sender, EventArgs e)
		{
			if (!base.IsPostBack)
			{
				this.txtUserType.Value = "2";
				PubFunction.BindShopSelectByShopType(this._UserShopID, this.sltShop, true, 2);
				PubFunction.BindAuthoritySelelctByGroupType(this._UserGroupID, this.sltUserGroupID, true, 2);
				PubFunction.BindShopSelectByShopType(this._UserShopID, this.sltShopInfo, false, 2);
		...
		private void GetSysUserList(string strSql)
		{
			...
			strSql += " and SysShop.ShopType=2 and SysUser.UserShopID = SysShop.ShopID and SysUser.UserGroupID = SysGroup.GroupID ";
			...
			if (this._UserGroupID != 1)
			{
				...
				strSql = string.Concat(new object[]
				{
					...
					" and  ((SysUser.UserID=",
					this._UserID,
					") or  (UserGroupID in (select GroupID from SysGroup where ParentIDStr like '%/",
					this.txtUserType.Value,
					"/%' or GroupID=",
					this.txtUserType.Value,
					")))"
				});
			...
			else
			{
				...
				{
					text,
					" and UserGroupID in (select GroupID from SysGroup where ParentIDStr like '%/",
					this.txtUserType.Value,
					"/%' or GroupID=",
					this.txtUserType.Value,
					")"

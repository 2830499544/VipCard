...
function BindGoodsList() {
    ...
    for (var i = 0; i < GoodsList.length; i++) {
        var html = '<tr class="td" >'
                 ...
                 + '<td><input id="txtBidPrice' + GoodsList[i].GoodsID + '"  type="text" class="border_radius common_ServiceButton border_radius2" style="text-align: right" value="' + parseFloat(GoodsList[i].InMoney).toFixed(2) + '" onkeyup="javascript:GoodsUpdate(' + GoodsList[i].GoodsID + ');" onclick=\"javascript:this.select();\"/></td>'
                      ...
                      default:
                          art.dialog
                                ({
                                    ...
                                    time: 0.5,
                                    content: '商品入库成功！',
                                    close: function () { PrintGoodsIn($("#lblUSer").html(), $("#sltShop").find("option:selected").text(), GoodsList, $("#chkPrint").attr("checked"), PointNum); },
                                    lock: true
                                });
                          break;
                  }

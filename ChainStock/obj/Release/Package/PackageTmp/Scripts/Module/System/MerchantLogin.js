...
function Login_ChangeValImg() {
    /*自动刷新验证码*/
    ChangeValImg();
    setTimeout("Login_ChangeValImg()", 10 * 60 * 1000);
}
...
function Login_Submit() {
    ...
    var account = $("#txtMerchant").val() + "_" + $("#txtAccount").val();
    doAjax("", "CheckUserLogin", { "account": account, "password": $("#txtPassword").val(), "valcode": valCode }, "json",
        function (json) {
            switch (json) {
                case 0:
                    ChangeValImg();
                    ...
                    break;
                case 1:
                    setTimeout("window.location = 'index.aspx'", 200);
                    break;
                case 2:
                    art.dialog
                          ({
                              title: '系统提示',
                              content: ("账号或商家处于锁定状态 请与系统管理员联系！"),
                              lock: true,
                              time: 2
                          });
                    break;

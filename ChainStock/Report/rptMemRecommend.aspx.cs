...
{
	public class rptMemRecommend : PageBase
	{
		...
		private void Query()
		{
			...
			using (DataTable dtMem = this.member.GetListSP(this.NetPagerParameter.PageSize, this.NetPagerParameter.CurrentPageIndex, out Counts, condition.ToArray()).Tables[0])
			{
		...
		protected void gvMemList_ItemDataBound(object sender, RepeaterItemEventArgs e)
		{
			if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
			{
				...
				int memid = int.Parse(dr["MemID"].ToString());
				if (memid >= 1)
				{
					Repeater rptItem = (Repeater)e.Item.FindControl("rptExpenseDetail");
					if (rptItem != null)
					{
						using (DataTable dtDetail = this.member.getMemRecommend(memid))
						{

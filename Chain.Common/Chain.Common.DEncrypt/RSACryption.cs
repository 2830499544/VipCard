	...
	public class RSACryption
	{
		...
		public string RSACreateRegisterCode(string privatekey, string encrypstriing)
		{
			...
			using (RSACryptoServiceProvider rSACryptoServiceProvider = new RSACryptoServiceProvider())
			{
				...
				byte[] bytes = Encoding.ASCII.GetBytes(encrypstriing);
				SHA1Managed sHA1Managed = new SHA1Managed();
				byte[] array = sHA1Managed.ComputeHash(bytes);
				Convert.ToBase64String(array);
				byte[] inArray = rSAPKCS1SignatureFormatter.CreateSignature(array);
		...
		public bool RSAVerifyRegisterCode(string publickey, string registercode, string registerusername)
		{
			...
			using (RSACryptoServiceProvider rSACryptoServiceProvider = new RSACryptoServiceProvider())
			{
				try
				{
					...
					byte[] rgbSignature = Convert.FromBase64String(registercode);
					SHA1Managed sHA1Managed = new SHA1Managed();
					byte[] rgbHash = sHA1Managed.ComputeHash(Encoding.ASCII.GetBytes(registerusername));
					result = rSAPKCS1SignatureDeformatter.VerifySignature(rgbHash, rgbSignature);
				}
				catch
				{
					result = false;

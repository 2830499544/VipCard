...
namespace Tencent
{
	internal class Cryptography
	{
		public static uint HostToNetworkOrder(uint inval)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				outval = (outval << 8) + (inval >> i * 8 & 255u);
			}
			return outval;
		}
		...
		public static string AES_decrypt(string Input, string EncodingAESKey, ref string appid)
		{
			...
			byte[] btmpMsg = Cryptography.AES_decrypt(Input, Iv, Key);
		...
		public static string AES_encrypt(string Input, string EncodingAESKey, string appid)
		{
			...
			byte[] Iv = new byte[16];
			Array.Copy(Key, Iv, 16);
			string Randcode = Cryptography.CreateRandCode(16);
			byte[] bRand = Encoding.UTF8.GetBytes(Randcode);
			byte[] bAppid = Encoding.UTF8.GetBytes(appid);
			byte[] btmpMsg = Encoding.UTF8.GetBytes(Input);
			byte[] bMsgLen = BitConverter.GetBytes(Cryptography.HostToNetworkOrder(btmpMsg.Length));
		...
		private static string CreateRandCode(int codeLen)
		{
			...
			string[] arr = codeSerial.Split(new char[]
			{
				','
			});
			string code = "";
		...
		private static string AES_encrypt(byte[] Input, byte[] Iv, byte[] Key)
		{
			...
			byte[] pad = Cryptography.KCS7Encoder(Input.Length);
		...
		private static byte[] KCS7Encoder(int text_length)
		{
			...
			char pad_chr = Cryptography.chr(amount_to_pad);
			string tmp = "";
		...
		private static byte[] AES_decrypt(string Input, byte[] Iv, byte[] Key)
		{
			...
			using (MemoryStream ms = new MemoryStream())
			{
				...
				xBuff = Cryptography.decode2(ms.ToArray());
			}
			return xBuff;
		}
